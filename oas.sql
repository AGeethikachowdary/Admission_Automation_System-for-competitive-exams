...
(`aid123`, `aishwarya`, `aish16`, `aishwaryalp166@gmail.com`);
...
-- Table for student_scores
CREATE TABLE IF NOT EXISTS `student_scores` (
    `student_Name` VARCHAR(255),
    `student_id` VARCHAR(10),
    `marks_in_physics` INT,
    `marks_in_chemistry` INT,
    `marks_in_maths` INT,
    `marks_in_biology` INT,
    `comedk_score` INT,
    `rank` INT,
    `confirmed_seat` TINYINT(1) DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS `colleges` (
    ...
    `college_details` VARCHAR(255) NOT NULL,
    `cutoff` INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS `branches` (
    ...
    `seats_in_branch` INT NOT NULL,
    `available_seats` INT NOT NULL,
    CONSTRAINT `fk_branches_colleges` FOREIGN KEY (`college_id`) REFERENCES `colleges` (`college_id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

-- Table for student_preferences
CREATE TABLE IF NOT EXISTS `student_preferences` (
    `student_id` VARCHAR(10) NOT NULL,
    `college_id` INT NOT NULL,
    `branch_id` INT NOT NULL,
    PRIMARY KEY (`student_id`, `college_id`, `branch_id`),
    CONSTRAINT `fk_student_preferences_user` FOREIGN KEY (`student_id`) REFERENCES `t_user` (`s_id`),
    CONSTRAINT `fk_student_preferences_colleges` FOREIGN KEY (`college_id`) REFERENCES `colleges` (`college_id`),
    CONSTRAINT `fk_student_preferences_branches` FOREIGN KEY (`branch_id`) REFERENCES `branches` (`branch_id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

-- Table structure for table `t_comedk_application`
CREATE TABLE IF NOT EXISTS `t_comedk_application` (
  `s_student_id` varchar(15) NOT NULL,
  `s_applicant_name` varchar(50) NOT NULL,
  `s_date_of_birth` date NOT NULL,
  `s_unique_id_proof_type` varchar(20) NOT NULL,
  `s_unique_id_proof_number` varchar(50) NOT NULL,
  `s_unique_id_proof_file` varchar(200) NOT NULL,
  `s_mobile_number` varchar(15) NOT NULL,
  `s_email` varchar(50) NOT NULL,
  `s_applying_for` varchar(10) NOT NULL,
  `s_previous_allotment` varchar(5) NOT NULL,
  `s_gender` varchar(10) NOT NULL,
  `s_nationality` varchar(50) NOT NULL,
  `s_disability` varchar(5) DEFAULT NULL,
  `s_father_name` varchar(50) NOT NULL,
  `s_mother_name` varchar(50) NOT NULL,
  `s_father_number` varchar(15) NOT NULL,
  `s_mother_number` varchar(15) NOT NULL,
  `s_father_occupation` varchar(50) NOT NULL,
  `s_parents_income` varchar(20) NOT NULL,
  ...
  `s_caste_file` varchar(200) NOT NULL,
  `s_board_of_qualifying_exam` varchar(50) NOT NULL,
  `s_state_of_qualifying_exam` varchar(50) NOT NULL,
  `s_tenth_marks` varchar(10) NOT NULL,
  `s_twelfth_marks` varchar(10) NOT NULL,
  `s_board_marks_files` varchar(200) NOT NULL,
  `s_photo_file` varchar(200) NOT NULL,
  `s_signature_file` varchar(200) NOT NULL,
  PRIMARY KEY (`s_student_id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

ALTER TABLE t_comedk_application
ADD COLUMN exam_ID VARCHAR(10);
---------------
