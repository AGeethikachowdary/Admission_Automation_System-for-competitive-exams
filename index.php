...
if (isset($_REQUEST["u_sub"])) {
    ...
    if ($id != '' && $pwd != '') {
        $query = mysqli_query($con, "SELECT * FROM t_status WHERE s_id='$id' AND s_stat='Applied'");
        ...
        if ($res) {
            ...
            $query1 = mysqli_query($con, "SELECT * FROM t_user_data WHERE s_id='$id' AND s_pwd='$pwd'");
            $res1 = mysqli_fetch_row($query1);

            if ($res1) {
                $_SESSION['user'] = $id;
                header('location:admissionform2.php');
            } else {
                echo '<script>';
